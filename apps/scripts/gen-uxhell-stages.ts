...
const TYPE_EXPLAIN_WHY_BAD: Record<string, string[]> = {
  volume_hover_slider: [
    '호버로만 작동하는 슬라이더는 터치 디바이스에서 완전히 무용지물이 됩니다.',
    '마우스를 올려야만 나타나는 컨트롤은 발견 가능성(discoverability)을 제로로 만듭니다.',
    '호버 의존 UI는 접근성 지침(WCAG)을 위반하며, 운동 장애 사용자를 배제합니다.',
  ],
  ...
  volume_hidden_icon: [
    ...
    '아이콘의 가시성을 의도적으로 낮추는 것은 사용자를 기만하는 다크 패턴입니다.',
  ],
  volume_reverse_mapping: [
    '직관과 반대로 동작하는 UI는 사용자의 멘탈 모델을 완전히 파괴합니다.',
    '역방향 매핑은 사용자가 이미 학습한 인터랙션 패턴을 무력화시킵니다.',
  ...
  clutter_find_cta: [
    ...
    '광고와 진짜 버튼을 구분하기 어렵게 만드는 것은 사기성 다크 패턴입니다.',
    '정보 과부하는 인지 부하를 폭증시켜 의사결정을 마비시킵니다.',
    '어수선한 UI는 핵심 기능의 발견 가능성을 의도적으로 떨어뜨립니다.',
  ],
  ...
  nav_ambiguity_map: [
    ...
    '모호한 메뉴 레이블은 사용자에게 추측을 강요하며 오탐을 유발합니다.',
  ...
  label_ambiguity: [
    '모호한 버튼 레이블은 사용자가 어떤 결과가 일어날지 예측할 수 없게 만듭니다.',
    '"확인"과 "취소"가 뒤바뀐 다이얼로그는 심각한 데이터 손실을 유발할 수 있습니다.',
    '버튼 레이블이 동작을 정확히 반영하지 않으면 사용자는 클릭할 때마다 불안합니다.',
    '혼동스러운 레이블은 사용자의 신뢰를 근본적으로 훼손합니다.',
  ],
  consent_toggle_labour: [
    '"모두 거부" 없이 수십 개 토글을 개별 해제하게 하는 것은 의도적 동의 착취입니다.',
    'GDPR은 동의 거부를 동의 수락만큼 쉽게 해야 한다고 명시합니다.',
    '쿠키 동의 UI에서 토글 노동을 강요하면 대부분 사용자는 포기하고 "모두 허용"을 누릅니다.',
    '개별 토글 해제를 요구하는 것은 사용자의 프라이버시 권리를 침해하는 다크 패턴입니다.',
  ],
  modal_stack: [
    '팝업 위에 팝업이 쌓이면 사용자는 원래 작업으로 돌아갈 수 없게 됩니다.',
    '겹쳐진 모달은 화면을 가리고 사용자의 통제감을 완전히 빼앗습니다.',
    '모달 스택은 사용자의 작업 흐름을 강제로 중단시키는 가장 공격적인 패턴입니다.',
    '닫기 어려운 모달은 사용자를 가두어 원하지 않는 행동을 유도합니다.',
  ],
  roach_motel_flow: [
    '가입은 쉽고 탈퇴는 어렵게 만드는 비대칭 UX는 사용자 신뢰를 파괴합니다.',
    '탈퇴 버튼을 숨기거나 복잡한 절차를 요구하는 것은 로치 모텔 패턴의 전형입니다.',
    '해지 과정에 불필요한 단계를 추가하는 것은 FTC가 규제하는 다크 패턴입니다.',
    '구독 취소를 의도적으로 어렵게 만들면 사용자 이탈 대신 소송을 얻게 됩니다.',
  ],
  hidden_reject_link: [
    '거절 옵션을 숨기면 사용자는 원치 않는 서비스에 강제 동의하게 됩니다.',
    '화려한 CTA 옆에 투명한 거절 링크를 배치하는 것은 대표적 기만형 다크 패턴입니다.',
  ...
  disguised_cta_grid: [
    ...
    '위장된 CTA는 사용자를 원치 않는 페이지로 유도해 시간을 낭비시킵니다.',
  ],
  state_feedback_broken: [
    '오류 메시지 없이 유효성 검사를 하면 사용자는 뭘 고쳐야 할지 알 수 없습니다.',
    '상태 피드백이 없는 폼은 사용자가 시행착오로만 올바른 입력값을 추측하게 합니다.',
    '진행 상태가 보이지 않는 양식은 사용자에게 극도의 불안감과 좌절감을 줍니다.',
    '피드백 없는 인터페이스는 닐슨의 10가지 휴리스틱 중 "시스템 상태의 가시성"을 위반합니다.',
  ],
  moving_target: [
  ...
  tiny_button: [
    '초소형 버튼은 피츠의 법칙에 따라 클릭 난이도를 기하급수적으로 높입니다.',
    '터치 타겟이 48dp 미만이면 모바일에서 사실상 사용 불가능합니다.',
    '의도적으로 작은 버튼은 사용자가 원하는 선택을 방해하려는 다크 패턴입니다.',
