...
const TYPE_EXPLAIN_WHY_BAD: Record<string, string[]> = {
  ...
  disguised_cta_grid: [
    '광고를 실제 게시물처럼 위장하면 사용자의 신뢰를 근본적으로 훼손합니다.',
    '진짜 후기와 네이티브 광고를 구분 불가능하게 만드는 것은 기만적 UI 패턴입니다.',
    '위장된 광고는 사용자를 원치 않는 구매로 유도해 시간과 돈을 낭비시킵니다.',
  ],
  moving_target: [
...
const TYPE_OBJECTIVES: Record<string, string[]> = {
  ...
  disguised_cta_grid: ['피드에서 진짜 게시물을 광고 속에서 구별하세요', '네이티브 광고와 진짜 게시물을 구분하세요', '위장된 광고 중 진짜 콘텐츠를 클릭하세요', '광고처럼 생긴 진짜 게시물을 찾으세요', '위장 광고 피드에서 진짜 콘텐츠를 고르세요'],
...
const TYPE_MEME_CAPTIONS: Record<string, string[]> = {
  ...
  disguised_cta_grid: [
    "후기인 줄 알았는데 광고였습니다.",
    "진짜 게시물과 네이티브 광고, 구별할 수 있나요?",
    "클릭하면 광고, 안 하면 못 넘어가는 피드.",
    "진짜 콘텐츠와 광고의 경계가 사라진 피드입니다.",
    "Sponsored 뱃지가 5픽셀 크기로 숨어있습니다.",
  ],
  label_ambiguity: [
    "확인이 취소이고 취소가 확인인 세계.",
...
function generateTypeTitle(type: string, params: Record<string, unknown>, hash: number): string {
  switch (type) {
    ...
    case 'disguised_cta_grid': {
      ...
      const titles = [
        size != null ? `${size}칸 피드: 진짜 게시물 찾기` : '위장 광고 피드 도전',
        fakes != null ? `${fakes}개 광고 중 진짜 게시물 찾기` : '네이티브 광고 구별하기',
        '피드 속 숨겨진 광고 피하기',
        '위장 광고 피드 정복',
      ];
      return pickFrom(titles, hash);
