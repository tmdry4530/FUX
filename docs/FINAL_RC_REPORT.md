...
### 개요
20개 UX 지옥 레퍼런스(Volume 10 + Web 10)를 16개 스테이지(타입당 1개)로 통합.
매 도전마다 목표치가 랜덤으로 변경되어 무한 리플레이 가능.

### Pack 구성
- **Volume Hell**: 10 levels (타입당 1개, 목표 볼륨 랜덤)
- **Web Hell**: 6 levels (타입당 1개, 목표 액션/라벨/단계수 랜덤)
...
### 난이도
모든 V3 스테이지는 단일 난이도(Normal)로 통합. 매 플레이마다 목표가 랜덤으로 변경되어 자연스러운 난이도 변동 제공.
...
62 modules, 282.47 kB (gzip: 88.20 kB)
ID 검증: 중복 0, 공백 0, 인코딩 필요 0
총 레벨: v3=16 + v2=20 + legacy=10 = 46
```
